...
public class PlatformerMotor2D : MonoBehaviour
{
    ...
    private void Start()
    {
		int charapter = PlayerPrefs.GetInt("Char");
		if (charapter == 1) {
			groundSpeed = groundSpeed + (5f / 2);
			jumpHeight = jumpHeight + (5f/15);
			wallJumpMultiplier = wallJumpMultiplier + (5f / 40); // При делении на 4 = +1.25 (125%). При делении на 40 = 0.25 (25%) 
		}
		if (charapter == 2) {
			groundSpeed = groundSpeed + (6f / 2);
			jumpHeight = jumpHeight + (6f/15);
			wallJumpMultiplier = wallJumpMultiplier + (6f / 40);
		}
